...
src-tauri/src/
  ...
  printer/escpos.rs       ESC/POS byte-building (tested); send_to_printer() auto-detects a USB printer (rusb) — serial/network still unimplemented
...
- **PDF is the real default receipt, not a placeholder**: `utils/receiptPdf.ts` (jsPDF + `jspdf-autotable`) builds an actual 80mm-thermal-shaped receipt and is dynamically `import()`-ed from `ReceiptModal` so its ~140KB chunk never loads on the billing screen itself. `printer/escpos.rs`'s `build_receipt_bytes` is complete and tested; `send_to_printer` (via `rusb`, `features = ["vendored"]` so libusb doesn't need to be preinstalled) auto-detects any attached USB device exposing a standard Printer-class (0x07) interface and writes the bytes to its bulk OUT endpoint — no vendor/product ID config needed. Serial and network (TCP 9100) transports are still unimplemented, so `PrinterError::NotConfigured` covers both "no USB printer attached" and "printer is serial/network-only." Always treat any error here as "fall back to PDF," never as a reason to fail a sale that's already committed.
